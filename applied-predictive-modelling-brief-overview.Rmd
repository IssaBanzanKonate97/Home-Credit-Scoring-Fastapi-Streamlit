...
setwd("C:/Users/PC/OneDrive/Documents/home-credit-default-risk")
library(data.table)
library(caret)
library(e1071)
library(pROC)
...
load.libraries <- c(
  'plyr','dplyr','data.table','readxl','reshape2','stringr','stringi',
  'ggplot2','tidyverse','gridExtra','matrixStats','lubridate','corrplot',
  'e1071','xgboost','caret','zoo','factoextra','plotly','DT','C50'
)
...
colnames(mv)[1] <- "missing_values"
mv <- cbind(Column = rownames(mv), mv)
mv <- setDT(mv)[order(missing_values, decreasing = TRUE)]
head(mv, 10)
...
probs <- predict(svmFitLinear, newdata = dtTest , type = "prob")
dtTest$svmProb_Yes <- probs$Yes
dtTest$svmProb_No <- probs$No


dtTest$logclass <- predict(logisticReg, dtTest)
logprobs <- predict(logisticReg, newdata = dtTest , type = "prob")
dtTest$logProb_Yes <- logprobs$Yes
dtTest$logProb_No  <- logprobs$No
...
```{r}
calCurve <- calibration(TARGET ~ svmProb_Yes + logProb_Yes, data = dtTest)
calCurve
...
# s'assurer que la cible est factor à 2 niveaux, "No" puis "Yes"
dtTest$TARGET <- factor(dtTest$TARGET, levels = c("No","Yes"))

# utiliser la colonne de proba créée plus haut
rocCurve <- pROC::roc(
  response  = dtTest$TARGET,
  predictor = dtTest$logProb_Yes,   # <- bonne colonne
  levels    = c("No","Yes")         # contrôles d'abord, cas ensuite
)
...
```r
```{r Logreg}
# --- ROC/AUC pour la régression logistique (robuste) ---
library(pROC)

# s'assurer que le test set contient bien les 2 classes
if (length(unique(dtTest$TARGET)) < 2) {
  set.seed(123)
  ok <- FALSE
  while (!ok) {
    inTrain <- caret::createDataPartition(dt1_preproc_sample$TARGET, p = .8)[[1]]
    dtTrain <- dt1_preproc_sample[ inTrain, ]
    dtTest  <- dt1_preproc_sample[-inTrain, ]
    ok <- length(unique(dtTest$TARGET)) == 2
  }
  # re-entraîner le modèle et recalculer les probabilités
  logisticReg <- caret::train(TARGET ~ ., data = dtTrain, method = "glm",
                              trControl = traincntrl)
  logprobs <- predict(logisticReg, newdata = dtTest, type = "prob")
  dtTest$logProb_Yes <- logprobs$Yes
}

# bons niveaux + bonne colonne de proba
dtTest$TARGET <- factor(dtTest$TARGET, levels = c("No","Yes"))

rocCurve <- pROC::roc(
  response  = dtTest$TARGET,
  predictor = dtTest$logProb_Yes,  # bonne proba
  levels    = c("No","Yes"),
  quiet     = TRUE
)
...
alltreemodels <- resamples(list(CART = dtFitCART,  
                                # PART = dtFitPART, 
                                Bagged = dtFitBagged,
                                RF = dtFitrf, XGBoost = dtFitXGboost,
                                C5.0 = dtFitC5.0))
...
```{r}
# classes prédites
dtTest$C5_0_class <- predict(dtFitC5.0, dtTest)

# probabilités prédites (2 colonnes : No / Yes)
C5_0_probs <- predict(dtFitC5.0, newdata = dtTest, type = "prob")
dtTest$C5_0_prob_No  <- C5_0_probs$No
dtTest$C5_0_prob_Yes <- C5_0_probs$Yes


# mêmes niveaux pour vérité terrain et prédictions
dtTest$TARGET      <- factor(dtTest$TARGET,      levels = c("No","Yes"))
dtTest$C5_0_class  <- factor(dtTest$C5_0_class,  levels = c("No","Yes"))

confusionMatrix(
  data      = dtTest$C5_0_class,   # <- nouveau nom
  reference = dtTest$TARGET,
  positive  = "Yes"
)
